...
| 任务 | 工时 | 负责 Agent | 依赖 |
|------|------|------------|------|
| 1.1 工作流持久化 | 5-7天 | Expert (Claude) 审核 → Backend (Codex) 实现 → Frontend (Gemini) Hook | Supabase |
| 1.2 测试体系基础 | 10-15天 | Backend (Codex) 后端测试 + Frontend (Gemini) 前端测试 + Expert (Claude) CI配置 | 无 |
| 1.3 Sentry 监控 | 2-3天 | Backend (Codex) 后端集成 + Frontend (Gemini) 前端集成 + Expert (Claude) 告警配置 | 无 |

### Agent 分工说明

| Agent | LLM | 本阶段任务 |
|-------|-----|------------|
| **Expert (Claude)** | Claude | 🔍 数据表审核、联调验收、CI配置、告警规则、最终验收 |
| **Backend (Codex)** | Codex | ⚡ SQL实现、后端API、pytest测试、Sentry后端集成 |
| **Frontend (Gemini)** | Gemini | 🎨 Hook实现、组件改造、vitest测试、Sentry前端集成 |
