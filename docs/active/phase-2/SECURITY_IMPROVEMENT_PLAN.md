...
| Agent | LLM | Phase 2 任务 |
|-------|-----|--------------|
| **Expert (Claude)** | Claude | 🔍 SEC-08 依赖安全扫描CI配置、所有安全任务最终验收 |
| **Backend (Codex)** | Codex | ⚡ SEC-01~07 全部后端安全实现 |
| **Frontend (Gemini)** | Gemini | 🎨 SEC-03 前端 Sentry 过滤配置 |

### 阶段 1: 紧急修复 (本周)

| 任务 | 优先级 | 工时 | 负责 Agent |
|------|--------|------|------------|
| SEC-01 CORS 修复 | P0 | 2h | **Backend (Codex)** → Expert (Claude) 验收 |
| SEC-06 日志过滤 | P1 | 3h | **Backend (Codex)** |
| SEC-05 基础限流 | P1 | 4h | **Backend (Codex)** |

### 阶段 2: v1.1 配套 (2 周内)

| 任务 | 优先级 | 工时 | 负责 Agent |
|------|--------|------|------------|
| SEC-03 Sentry 白名单 | P1 | 2h | **Joint**: Backend (Codex) 后端 + Frontend (Gemini) 前端 |
| SEC-04 JWT 增强 | P1 | 3h | **Backend (Codex)** |
| SEC-08 CI 安全扫描 | P2 | 4h | **Expert (Claude)** |

### 阶段 3: v1.2 配套 (1 月内)

| 任务 | 优先级 | 工时 | 负责 Agent |
|------|--------|------|------------|
| SEC-02 BYOK 加密 | P0 | 8h | **Backend (Codex)** → Expert (Claude) 安全审阅 |
| SEC-07 RLS 测试 | P2 | 6h | **Backend (Codex)** |
